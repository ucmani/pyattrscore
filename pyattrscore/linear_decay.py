"""
Linear Time Decay Attribution Model

This module implements the Linear Time Decay Attribution model, which
credits interactions linearly based on recency within the attribution window.
More recent touchpoints receive higher credit in a linear fashion.
"""
...
class LinearDecayAttribution(AttributionModel):
    """
    Linear Time Decay Attribution Model.
    
    This model assigns attribution credit based on a linear decay function,
    where touchpoints closer to conversion receive linearly higher credit.
    The decay rate controls how much the attribution decreases over time.
    
    Key characteristics:
    - Recent touchpoints receive higher credit
    - Linear decay function provides gradual weighting
    - Configurable decay rate
    - Respects attribution window settings
    - Easier to understand than exponential decay
    """
    
    def __init__(self, config: AttributionConfig = None):
        """
        Initialize Linear Decay Attribution model.
        ...
        logger.info(f"Initialized Linear Decay Attribution model with "
    ...
    def calculate_attribution(self, data: pd.DataFrame) -> pd.DataFrame:
        """
        Calculate linear decay attribution scores.
        
        This method assigns attribution based on linear time decay,
        ...
        """
        logger.info("Starting Linear Decay Attribution calculation")
        ...
        try:
            ...
            logger.info(f"Linear Decay Attribution completed successfully. "
            ...
            logger.error(f"Unexpected error in Linear Decay Attribution: {str(e)}")
            raise AttributionCalculationError(
                f"Failed to calculate Linear Decay Attribution: {str(e)}",
    ...
    def _calculate_user_attribution(self, touchpoints: List[TouchpointData]) -> List[Dict[str, Any]]:
        """
        Calculate attribution for a single user's journey using linear decay.
        
        For Linear Decay Attribution, we:
        1. Filter touchpoints within the attribution window
        2. Calculate linear decay weights based on time to conversion
        ...
        logger.debug(f"Calculated linear decay attribution for user with {len(touchpoints)} touchpoints, "
    ...
    def _validate_results(self, result_df: pd.DataFrame) -> None:
        ...
        logger.debug("Linear Decay Attribution results validation passed")
    
    def get_model_info(self) -> Dict[str, Any]:
        """
        Get information about the Linear Decay Attribution model.
        ...
        model_specific_info = {
            'attribution_logic': 'Assigns credit based on linear decay function with recency bias',
            'use_cases': [
                'Understanding gradual recency impact',
                'Balanced time-based weighting',
                'Alternative to exponential decay'
            ],
            'advantages': [
            ...
            'limitations': [
                ...
                'Less sophisticated than exponential decay'
            ],
